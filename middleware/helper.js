var self={
	...
	getMsgs:function(data,connection,callback){
		...
		*/
		var data={
			query:"select reply as msg,from_id,to_id,timestamp,photo from conversation_reply where from_id='"+data.from_id+"' and to_id='"+data.uid+"' or  from_id='"+data.uid+"' and to_id='"+data.from_id+"' order by timestamp asc",
			connection:connection
		}
		self.queryRunner(data,function(result){
			if(result.length > 0){
